...
module.exports = function lostColumnDecl(css, settings, result) {
  css.walkDecls('lost-column', function lostColumnFunction(decl) {
    ...
    if (decl.parent.parent.params === 'only screen and (min-width: 768px)') {
      lostColumnGutter = settings.gutterTablet;
    }

    if (decl.parent.parent.params === 'only screen and (min-width: 980px)') {
      lostColumnGutter = settings.gutterDesktop;
    }
